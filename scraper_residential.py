...
def parse_listing(article) -> Optional[dict]:
    try:
        ...
        return {
            ...
            "published_date":    None,   # uzupełniane z __NEXT_DATA__ (realna data publikacji)
        }
...
def extract_published_dates(html: str) -> dict:
    """Z __NEXT_DATA__ (props.pageProps.data.searchAds.items) buduje
    mapę {offer_id: dateCreated}. offer_id z sufiksu -IDxxx w slug."""
    import json
    out = {}
    ...
    for it in items:
        ...
        # 1999-02-29 = sentinel Otodom "brak daty" (i nieistniejąca data) → pomijamy
...
def scrape(url: str, headless: bool = True) -> tuple:
    ...
    with sync_playwright() as p:
        ...
        for page_num in range(1, 30):
            ...
            # Realna data publikacji z embedded JSON (A2) — raz na stronę
            try:
    ...
    with get_conn() as conn:
        ...
        for offer in offers:
            ...
            if not existing:
                ...
                insert_lifecycle_event(conn, oid, scrape_date, "LISTING_CREATED",
                                       None, new_price)
            else:
                # RELISTED — oferta wróciła po delistingu
            ...
            # Persist lifecycle: published_date (realna data, nie nadpisuj jeśli już jest),
            # status, last_known_price; wyczyść delisted_date przy relistingu
            conn.execute("""
                UPDATE listings SET
